...
static void handle_new_connection(int server_fd, int epoll_fd) {
    ...
    event.events = EPOLLIN;  // Level-triggered mode (removed EPOLLET)
...
static int read_message(int client_fd, message_t **msg) {
    ...
    while (total_read < sizeof(header)) {
        ssize_t n = recv(client_fd, ((char*)&header) + total_read, 
                         sizeof(header) - total_read, 0);
        if (n == 0) {
    ...
    *(int32_t*)((*msg)->body) = client_fd;
    ...
    while (total_read < header.length) {
        ssize_t n = recv(client_fd, (*msg)->body + CLIENT_FD_SIZE + total_read, 
                         header.length - total_read, 0);
        if (n == 0) {
...
static void handle_client_data(int client_fd, queue_t *msg_queue) {
    ...
    // result == 0 means no data available yet (EAGAIN), just continue
}
