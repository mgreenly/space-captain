...
static char* generate_random_message(message_type_t type) {
    static const char* echo_messages[] = {
        "Hello, server!",
    ...
    static const char* reverse_messages[] = {
        "Reverse this string",
        "!gnirts siht esreveR",
        "The quick brown fox",
    ...
    static const char* time_messages[] = {
        "What time is it?",
        "Current time please",
        "Time check",
...
static int send_message(int sock, message_type_t type, const char* body) {
    ...
    log_debug("Request details - Type: %d (%s), Length: %u bytes", 
              type, type_str, header.length);
    return 0;
}
...
static int receive_response(int sock) {
    ...
    log_debug("Response details - Type: %d (%s), Length: %u bytes", 
              header.type, type_str, header.length);
