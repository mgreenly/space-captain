...
static bool check_for_errors(int pipe_fd, const char *process_name) {
  ...
  while ((bytes_read = read(pipe_fd, buffer, sizeof(buffer) - 1)) > 0) {
    ...
    if (strstr(buffer, "ERROR") || strstr(buffer, "Error") || strstr(buffer, "error") ||
        strstr(buffer, "FAILED") || strstr(buffer, "Failed") || strstr(buffer, "failed")) {
...
void test_server_client_interaction(void) {
  ...
  TEST_ASSERT_MESSAGE(access("bin/server", X_OK) == 0, "Server binary not found or not executable");
  TEST_ASSERT_MESSAGE(access("bin/client", X_OK) == 0, "Client binary not found or not executable");
  ...
  // Assert success
  TEST_ASSERT_TRUE_MESSAGE(success, "Test failed: either processes died or errors were detected");
}
